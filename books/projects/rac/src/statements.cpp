...
Statement *Statement::subst([[maybe_unused]] SymRef *var,
                            [[maybe_unused]] Expression *expr) { // virtual
  return this;
}
...
void Statement::noteReturnType(
    [[maybe_unused]] Type
        *t) { // virtual (overridden by Block, ReturnStmt, and IfStmt)
}

void Statement::markAssertions([
    [maybe_unused]] FunDef *f) { // virtual (overridden by Assertion)
}
...
Sexpression *EnumConstDec::ACL2SymExpr() {
  return ((const EnumType *)type)->getEnumVal(sym);
}
...
Statement *ConstDec::subst([[maybe_unused]] SymRef *var,
...
void ReturnStmt::noteReturnType(Type *t) { returnType = t; }
...
void Assertion::markAssertions(FunDef *f) { funDef = f; }
...
void ForStmt::markAssertions(FunDef *f) { body->markAssertions(f); }
